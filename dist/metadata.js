    ...
    static getAuthor(doc, schemaOrgData, metaTags) {
        ...
        if (authorsString) {
            ...
            if (/^https?:\/\//i.test(authorsString.trim())) {
                ...
                if (cleaned)
                    return { name: cleaned, ...(authorUrl ? { url: authorUrl } : {}) };
            }
        }
        ...
        if (relAuthorEls.length > 0 && relAuthorEls.length <= 3) {
            ...
            if (uniqueRelNames.length > 0)
                return { name: uniqueRelNames.join(', '), ...(authorUrl ? { url: authorUrl } : {}) };
        }
        ...
        for (const { selector, maxMatches } of domAuthorSelectors) {
            ...
            matches.forEach(el => {
                ...
                if (!authorUrl && el instanceof HTMLAnchorElement && el.href) {
                    ...
                    if (link)
                        authorUrl = link.getAttribute('href') || undefined;
                }
            });
        }
        ...
        if (h1) {
            ...
            for (let depth = 0; depth < 3 && bylineScope; depth++) {
                ...
                for (let i = 0; i < 3 && bylineCandidate; i++) {
                    ...
                    if (bylineResult)
                        return { name: bylineResult, ...(authorUrl ? { url: authorUrl } : {}) };
